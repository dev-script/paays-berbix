const message = {
    ACCESS_DENIED: "Access denied",
    INTERNAL_SERVER_ERROR: 'something went wrong',
    INVALID_REQUEST_ID: 'invalid request id',
    ID_NOT_FOUND: 'id not found',
    USER_NOT_FOUND: 'user not found',
    CREATED_SUCCESSFULLY: 'created successfully',
    UPDATED_SUCCESSFULLY: 'updated successfully',
    DELETED_SUCCESSFULLY: 'deleted successfully',
    DATA_ALREADY_EXISTS: 'data already exists',

    // USER
    USER_DOES_NOT_EXIST: 'user does not exists',
    USERNAME_REQUIRED: 'username required',
    NAME_REQUIRED: 'name required',
    USER_PASSWORD_REQUIRED: 'user password required',
    PLEASE_CHECK_INPUT_LENGTH: 'please check input length',
    PASSWORD_NOT_ACCEPTED: 'password not accepted',
    INVALID_USERNAME_PASSWORD: 'invalid username/password',
    USER_ALREADY_EXISTS: 'user already exists',
    INVALID_PASSWORD_PATTERN: 'invalid password pattern',
    USER_SIGN_UP_SUCCESSFULLY: 'signup successfully',
    USER_LOGIN_SUCCESSFULLY: 'login successfully',
    SUBSCRIPTION_START_DATE_REQUIRED: 'subscription start date required',
    INVALID_SUBCRIPTION_START_DATE: 'invalid subscription start date',
    SUBSCRIPTION_END_DATE_REQUIRED: 'subscription end date required',
    INVALID_SUBCRIPTION_END_DATE: 'invalid subscription end date',
    INVALID_PHONE_NUMBER: "invalid phone number",
    PHONE_NUMBER_REQUIRED: "phone number is required",

    PLEASE_PROVIDE_PHONE_NUMBER: 'please provide phone number',
    INVALID_EMAIL: 'invalid email',
    TRANSACTION_ID_REQUIRED: 'transaction id is required',
};

module.exports = {
    message,
};
